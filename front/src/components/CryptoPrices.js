...
const CryptoCharts = lazy(() => import("./CryptoCharts"));
...
function CryptoPrices({ onPredictClick }) {
  ...
  const fetchMarketData = async () => {
    try {
      ...
      const gainersResponse = await fetch('https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=100&page=1&price_change_percentage=24h');
        ...
        <div key={key} className="swiper-slide">
          <div className="items-wrapper">
            <div className="market-more-item">
              ...
              <button onClick={onPredictClick} className="stack-button">
                Predict
              </button>
            </div>
          </div>
